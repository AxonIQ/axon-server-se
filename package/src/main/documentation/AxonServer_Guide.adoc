...
You can use a single AxonDB (cluster) to store events for multiple bounded contexts. Each context will have its own
...
[subs="attributes"]
```
{CliCmd} register-context -S http://axondb1:8023 -c context-name
```

The server address here is the address of the master for the *default* context. It is not possible to delete the *default*
context.

=== Multi-tier storage [Enterprise Edition only]

AxonDB Enterprise Edition supports storing older files in a different location. It is for instance possible to store the
first 100 segments in one location (using SSD disks) and move all the older segments to another location, on cheaper storage.

For this there are 2 options:

1. keep all files for each node in the cluster on the node itself
2. move older files to a shared location

To set-up the first option add the following properties to the axondb.properties file:

```
axoniq.axondb.file.maxFilesInPrimaryLocation=100
axoniq.axondb.file.alternateStorage=/test/oldEvents
```

For the second option, use these properties:
```
axoniq.axondb.file.maxFilesInPrimaryLocation=100
axoniq.axondb.file.sharedStorage=/test/sharedEvents
```

=== Migration

The {ProductName} package contains a migration tool to migrate from an already existing RDBMS event store to a new {ProductName} instance.
The tool reads events and snapshots from the existing store and pushes them to the {ProductName} server.

The migration tool maintains the state of its migration, so it can be run multiple times.

Set the following properties to define the existing event store and the target {ProductName} server:

* *axoniq.axondb.servers* - comma separated list of hostnames and ports for the {ProductName} cluster.
...
Note that the migration tool only migrates the event store data to AxonDB. It does not update the tracking token values in
...
|axon.axonserver.keep-alive-time|0|Long|Interval for keep alive messages, 0 disables keep alive messages
|axon.axonserver.keep-alive-timeout|5000|Long|Timeout for keep alive messages, only of keep-alive-time is greater than 0
|===========================
