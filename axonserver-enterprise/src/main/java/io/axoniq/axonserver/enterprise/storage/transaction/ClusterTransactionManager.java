...
public class ClusterTransactionManager implements StorageTransactionManager {
    ...
    public CompletableFuture<Long> store(List<SerializedEvent> eventList) {
        ...
        activeTransactions.put( preparedTransaction.getToken(),
                                new ActiveTransactionInformation( completableFuture, replicationManager.getQuorum(type.getContext()), eventList.size()));
        try {
    ...
    private class ActiveTransactionInformation {
        ...
        public ActiveTransactionInformation(CompletableFuture<Long> storageCallback, int quorum, int eventCount) {
