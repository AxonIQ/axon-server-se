...
public class RemoteRaftGroupService implements RaftGroupService {
    private static final Function<InstructionResult, Void> TO_VOID = x -> null;
    ...
    public CompletableFuture<Void> deleteContext(String context) {
        ...
        stub.deleteContext(ContextName.newBuilder()
                                      .setContext(context)
                                      .build(), new StreamObserver<InstructionResult>() {
            @Override
            public void onNext(InstructionResult value) {
                result.complete(null);
            }
