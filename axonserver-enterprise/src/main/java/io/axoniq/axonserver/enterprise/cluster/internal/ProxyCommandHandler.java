...
public class ProxyCommandHandler extends CommandHandler<ConnectorResponse> {
    private static final InstructionAck confirmationBase = InstructionAck.newBuilder().setSuccess(true).build();
    ...
    public void confirm(String messageId) {
        observer.onNext(ConnectorResponse.newBuilder().setConfirmation(InstructionAck.newBuilder(confirmationBase).setInstructionId(messageId)).build());
    }
