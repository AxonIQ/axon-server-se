...
public class ProxyCommandHandler extends CommandHandler<ConnectorResponse> {
    private static final InstructionResult confirmationBase = InstructionResult.newBuilder().setSuccess(true).build();
    ...
    public void confirm(String messageId) {
        observer.onNext(ConnectorResponse.newBuilder().setConfirmation(InstructionResult.newBuilder(confirmationBase).setInstructionId(messageId)).build());
    }
