...
public class GrpcRaftGroupService extends RaftGroupServiceGrpc.RaftGroupServiceImplBase {
    ...
    private void confirm(StreamObserver<InstructionResult> responseObserver, CompletableFuture<Void> completable) {
        completable.whenComplete((r, t) -> {
            if (t != null) {
                responseObserver.onError(t);
            } else {
                responseObserver.onNext(InstructionResult.newBuilder().setSuccess(true).build());
                responseObserver.onCompleted();
            }
        });
    }
    ...
    @Override
    public void appendEntry(ContextEntry request, StreamObserver<InstructionResult> responseObserver) {
    ...
    @Override
    public void mergeAppAuthorization(ContextApplication request, StreamObserver<InstructionResult> responseObserver) {
        CompletableFuture<Void> completable = localRaftGroupService.updateApplication(request);
        confirm(responseObserver, completable);
    }

    @Override
    public void deleteAppAuthorization(ContextApplication request, StreamObserver<InstructionResult> responseObserver) {
        CompletableFuture<Void> completable = localRaftGroupService.deleteApplication(request);
        confirm(responseObserver, completable);
    }

    @Override
    public void mergeUserAuthorization(ContextUser request, StreamObserver<InstructionResult> responseObserver) {
        CompletableFuture<Void> completable = localRaftGroupService.updateUser(request);
        confirm(responseObserver, completable);
    }

    @Override
    public void deleteUserAuthorization(ContextUser request, StreamObserver<InstructionResult> responseObserver) {
    ...
    public void mergeLoadBalanceStrategy(ContextLoadBalanceStrategy request,
                                         StreamObserver<InstructionResult> responseObserver) {
    ...
    @Override
    public void transferLeadership(ContextName request, StreamObserver<InstructionResult> responseObserver) {
