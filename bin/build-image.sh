...
IMG_VERSION=
IMG_FAMILY=
IMG_FAMILY_DEF=grpc-proxy
IMG_NAME=
IMG_USER=
IMG_USER_DEF=axonproxy
PROJECT=
...
cat > application-image.json <<EOF
{
  "builders": [
    {
      "type": "googlecompute",
  ...
  "provisioners": [
    ...
    {
        "type": "file",
        "source": "${MVN_MODULE}src/main/gce/application.yml",
    ...
    {
        "type": "file",
        "source": "${MVN_MODULE}src/main/gce/gen-cert.sh",
        "destination": "/tmp/gen-cert.sh"
    },
    {
      "type": "shell",
      "inline": [ "sudo yum -y update",
                  ...
                  "sudo cp /tmp/application.yml /opt/${IMG_USER}/",
                  "sudo cp /tmp/${IMG_FAMILY}.jar /opt/${IMG_USER}/",
                  "sudo cp /tmp/startup.sh /opt/${IMG_USER}/",
                  "sudo cp /tmp/gen-cert.sh /opt/${IMG_USER}/",
...
packer build -force -color=false application-image.json
