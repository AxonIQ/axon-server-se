...
/**
 * Finds all OSGi packages that are exported for the plugins to use. This includes the packages exported by the
 * OSGi bundles that are installed in Axon Server for configuration of the plugins and the plugin api packages.
 *
 * @author Marc Gathier
 * @since 4.5
 */
public class SystemPackagesProvider {
    ...
    private boolean export(String manifestUrl) {
        return manifestUrl.contains("org.osgi") ||
                manifestUrl.contains("axon-server-plugin-api") ||
                manifestUrl.contains("axonserver-plugin-api");
    }
}
