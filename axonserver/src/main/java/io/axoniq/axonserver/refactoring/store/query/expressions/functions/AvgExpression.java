...
public class AvgExpression extends AbstractAggregationFunction {
    ...
    public ExpressionResult apply(ExpressionContext expressionContext, ExpressionResult input) {
        ...
        AtomicReference<AvgValue> initialSum = scopedContext.computeIfAbsent(alias,
                                                                             () -> new AtomicReference<>(new AvgValue()));
    ...
    private static class AvgValue {
        ...
        public BigDecimal getAverage() {
            return count == 0 ? null : sum.divide(BigDecimal.valueOf(count), 8, RoundingMode.HALF_DOWN)
                                          .stripTrailingZeros();
        }
