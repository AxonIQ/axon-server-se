...
package io.axoniq.axonserver.refactoring.store.query;
...
import io.axoniq.axonserver.refactoring.store.EventStreamReader;
import io.axoniq.axonserver.refactoring.store.EventWriteStorage;
...
import io.axoniq.axonserver.refactoring.store.query.result.AbstractMapExpressionResult;
import io.axoniq.axonserver.refactoring.store.query.result.BooleanExpressionResult;
import io.axoniq.axonserver.refactoring.store.query.result.DefaultQueryResult;
import io.axoniq.axonserver.refactoring.store.query.result.EventExpressionResult;
...
import io.axoniq.axonserver.refactoring.store.query.result.TimestampExpressionResult;
            ...
            if (sender.start()) {
                ...
                if (queryEventsRequest.getLiveEvents() && maxToken > connectionToken) {
                    if (querySnapshots) {
                        ...
                        registration = eventWriteStorage.registerEventListener((token, events) -> pushEventFromStream(
                                token,
                                events,
                                pipeLine));
                    }
