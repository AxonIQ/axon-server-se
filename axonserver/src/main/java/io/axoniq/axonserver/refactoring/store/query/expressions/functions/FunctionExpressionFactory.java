...
import io.axoniq.axonserver.refactoring.store.query.ExpressionRegistry;
import io.axoniq.axonserver.refactoring.store.query.PipeExpression;
import io.axoniq.axonserver.refactoring.store.query.expressions.AbstractExpressionFactory;
import io.axoniq.axonserver.refactoring.store.query.expressions.AndExpression;
import io.axoniq.axonserver.refactoring.store.query.expressions.Identifier;
import io.axoniq.axonserver.refactoring.store.query.expressions.ListExpression;
import io.axoniq.axonserver.refactoring.store.query.expressions.NotExpression;
import io.axoniq.axonserver.refactoring.store.query.expressions.NumericLiteral;
import io.axoniq.axonserver.refactoring.store.query.expressions.OrExpression;
import io.axoniq.axonserver.refactoring.store.query.expressions.SortExpression;
...
public class FunctionExpressionFactory extends AbstractExpressionFactory {
            ...
            case "length":
                return Optional.of(new LengthExpression(element.alias().orElse("length"),
                                                        optionalParameter(0, element.getParameters(), registry)
                                                                .orElse(null)));
            case "or":
                return Optional.of(new OrExpression(element.alias().orElse(VALUE),
                                                    buildParameters(element.getParameters(), registry)));
            case "and":
                return Optional.of(new AndExpression(element.alias().orElse(VALUE),
                                                     buildParameters(element.getParameters(), registry)));
            case "not":
                return Optional.of(new NotExpression(element.alias().orElse(VALUE),
                                                     buildParameters(element.getParameters(), registry)));
            case SUBSTRING:
                return Optional.of(new SubstringExpression(element.alias().orElse(SUBSTRING),
                                                           buildParameters(element.getParameters(), registry)));
            case "concat":
                return Optional.of(new ConcatExpression(element.alias().orElse("concat"),
                                                        buildParameters(element.getParameters(), registry)));
            case "left":
                return Optional.of(new LeftExpression(element.alias().orElse("left"),
                                                      buildParameters(element.getParameters(), registry)));
            case "right":
                return Optional.of(new RightExpression(element.alias().orElse("right"),
                                                       buildParameters(element.getParameters(), registry)));
            case MATCH:
                return Optional.of(new MatchExpression(element.alias().orElse(MATCH),
                                                       buildParameters(element.getParameters(), registry)));
            case FORMATDATE:
                return Optional.of(new FormatDateExpression(element.alias().orElse(FORMATDATE),
                                                            buildParameters(element.getParameters(), registry)));
            case DAY:
                return Optional.of(new FormatDateExpression(element.alias().orElse(DAY),
                                                            buildParameters(element.getParameters(), registry)[0],
                                                            new StringLiteral("F")));
            case WEEK:
                return Optional.of(new FormatDateExpression(element.alias().orElse(WEEK),
                                                            buildParameters(element.getParameters(), registry)[0],
                                                            new StringLiteral("w")));
            case MONTH:
                return Optional.of(new FormatDateExpression(element.alias().orElse(MONTH),
                                                            buildParameters(element.getParameters(), registry)[0],
                                                            new StringLiteral("MM")));
            case YEAR:
                return Optional.of(new FormatDateExpression(element.alias().orElse(YEAR),
                                                            buildParameters(element.getParameters(), registry)[0],
                                                            new StringLiteral("yyyy")));
            case HOUR:
                return Optional.of(new FormatDateExpression(element.alias().orElse(HOUR),
                                                            buildParameters(element.getParameters(), registry)[0],
                                                            new StringLiteral("HH")));
            case METADATA:
                return Optional.of(new MetaDataExpression(element.alias().orElse(null),
                                                          buildParameters(element.getParameters(), registry)));
            default:
        }
    ...
    public Optional<PipeExpression> buildPipeExpression(QueryElement element, ExpressionRegistry registry) {
        switch (element.operator().toLowerCase()) {
            ...
            case "limit":
                return Optional.of(new LimitExpression(optionalParameter(0, element.getParameters(), registry)
                                                               .orElse(new NumericLiteral("1000", "1000"))));
            case "sortby":
