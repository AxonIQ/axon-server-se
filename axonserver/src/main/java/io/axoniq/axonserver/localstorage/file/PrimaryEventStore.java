...
public class PrimaryEventStore extends SegmentBasedEventStore {
    ...
    public Runnable reserveSequenceNumbers(List<SerializedEvent> events, boolean force) {
        ...
        Runnable unreserve = () -> oldSequenceNumberPerAggregate
                .forEach((aggregateId, sequenceNumber) ->
                                 sequenceNumbersPerAggregate.put(aggregateId, new AtomicLong(sequenceNumber)));
    ...
    private AtomicLong lastSequenceForAggregate(String aggregateId, boolean checkAll) {
        return new AtomicLong(getLastSequenceNumber(aggregateId,
                                                    checkAll ? Integer.MAX_VALUE : MAX_SEGMENTS_FOR_SEQUENCE_NUMBER_CHECK)
                                      .orElse(-1L));
    }
