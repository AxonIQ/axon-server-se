...
public class QueryTest {
    ...
    public void setup() {
        ...
        events.add(EventWithToken.newBuilder().setEvent(Event.newBuilder()
                                                             ...
                                                             .setPayload(SerializedObject.newBuilder()
                                                                                         ...
                                                                                         .build())
                                                             .putMetaData("meta-key",
                                                                          MetaDataValue.newBuilder()
                                                                                       .setTextValue("text-value")
                                                                                       .build())
                                                             .putMetaData("otherkey",
                                                                          MetaDataValue.newBuilder().setNumberValue(42L)
                                                                                       .build())
        ...
        events.add(EventWithToken.newBuilder().setEvent(Event.newBuilder()
                                                             ...
                                                             .putMetaData("meta-key",
                                                                          MetaDataValue.newBuilder()
                                                                                       ...
                                                                                       .build())
                                                             .putMetaData("otherkey",
                                                                          MetaDataValue.newBuilder().setNumberValue(1L)
                                                                                       .build())
    ...
    public void groupByExpression2WithFilter() {
        List<QueryResult> results = executeQuery(
                "groupby(payloadType, count(aggregateSequenceNumber), max(aggregateSequenceNumber)) | count > 2 ");
        results.forEach(System.out::println);
    ...
    public void containsExpression() {
        List<QueryResult> results = executeQuery(
                "sortBy(aggregateType) | contains(aggregateType, aggregateSequenceNumber)");
        results.forEach(System.out::println);
        assertEquals(2, results.size());
        // TODO: Validate result contents
    }
