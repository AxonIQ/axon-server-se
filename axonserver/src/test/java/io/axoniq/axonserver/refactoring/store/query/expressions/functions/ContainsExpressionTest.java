...
public class ContainsExpressionTest {
    ...
    public void setUp() {
        ...
        testSubject = new ContainsExpression("contains",
                                             new Expression[]{new Identifier("value"), new StringLiteral("xyz")});
    }

    @Test
    public void testStringContainsGivenValue() {
        ExpressionResult actualResult = testSubject.apply(expressionContext,
                                                          mapValue("value", stringValue("someValue_xyz_Containing")));
    ...
    public void testContainsMatchesElementInList() {
        ExpressionResult actualResult = testSubject.apply(expressionContext,
                                                          mapValue("value",
                                                                   listValue(stringValue("xyz"), numericValue(1L))));
        assertTrue(actualResult.isTrue());
    }
    ...
    public void testContainsDoesntMatchElementInList() {
        ExpressionResult actualResult = testSubject.apply(expressionContext,
                                                          mapValue("value",
                                                                   listValue(stringValue("someValue_xyz_Containing"),
                                                                             numericValue(1L))));
        assertFalse(actualResult.isTrue());
    }
    ...
    public void testContainsMatchesNumericValueInCollection() {
        testSubject = new ContainsExpression("contains",
                                             new Expression[]{new Identifier("value"), new NumericLiteral("nr", "1")});
