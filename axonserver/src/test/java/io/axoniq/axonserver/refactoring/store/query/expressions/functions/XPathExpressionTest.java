...
import io.axoniq.axonserver.refactoring.store.query.result.XmlExpressionResult;
import org.junit.*;
...
public class XPathExpressionTest {
    ...
    private String xmlDocument =
            "<io.axoniq.axondb.performancetest.axonapp.DataAddedToDummyEvt><index>12692</index><id>beff70ef-3160-499b-8409-5bd5646f52f3</id><tracker>175143</tracker><test>"
                    +
                    "<data>First</data>" +
                    "<data>Second</data>" +
                    "<data>Third</data>" +
    ...
    @Test
    public void apply() {
        ExpressionResult result = testSubject.apply(expressionContext, mapValue("test",
                                                                                stringValue(xmlDocument),
                                                                                "expression",
    ...
    public void applyWithNodes() {
        ExpressionResult result = testSubjectWithoutType.apply(expressionContext,
                                                               mapValue("test", stringValue(xmlDocument),
    ...
    public void getIndex() {
        ExpressionResult result = testSubjectWithoutType.apply(expressionContext,
                                                               mapValue("test",
                                                                        ...
                                                                        "expression",
                                                                        stringValue(
                                                                                "//io.axoniq.axondb.performancetest.axonapp.DataAddedToDummyEvt/index")));
        assertThat(result).isInstanceOf(StringExpressionResult.class);
