...
public class Cluster extends AxonIQCliCommand {
    public static void run(String[] args)  {
        ...
        try (CloseableHttpClient httpclient  = createClient(commandLine)) {
            ...
            System.out.printf("%6s %-40s %-40s %10s %10s %s\n", "Master", "Name", "Hostname", "gRPC Port", "HTTP Port", "Connected");
            Arrays.stream(nodes).sorted(Comparator.comparing(ClusterNode::getName)).forEach(node -> {
                System.out.printf("%6s %-40s %-40s %10d %10d %s\n", node.isMaster() ? "**" : "", node.getName(), node.getHostName(), node.getGrpcPort(), node.getHttpPort(), node.isConnected() ? "*" : "");
            });
